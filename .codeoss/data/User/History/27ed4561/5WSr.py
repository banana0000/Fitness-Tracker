...
def update_output_content(jsonified_cleaned_data):
    ...
    summary_and_charts_layout = html.Div([
        # Summary Row
        dbc.Row([
            dbc.Col(create_summary_card("Total Steps", f"{total_steps:,}", "light"), md=3),
            dbc.Col(create_summary_card("Average Daily Steps", f"{avg_steps:,.0f}", "light"), md=3),
            dbc.Col(create_summary_card("Total Calories Burned", f"{total_calories:,}", "light"), md=3),
            dbc.Col(create_summary_card("Best Day for Steps", best_day_text, "warning"), md=3),
        ], className="mb-4"),
