...
def dashboard_page(data):
    ...
    return dbc.Container(
        [
            ...
            dbc.Card(
                dbc.CardBody([
                    html.H5("📂 Upload Your CSV", className="text-warning"),
...
def insights_page(data):
    ...
    if diff_steps > 500:
        msg = "🔥 Yesterday you were more active than before — excellent job!"
        color = "success"
    elif diff_steps < -500:
        msg = "😴 A bit slow yesterday — plan a walk today!"
        color = "danger"
    else:
        msg = "💪 Consistent performance — stability is key."
        color = "warning"
...
def update_from_upload(contents, filename):
    ...
    try:
        ...
        msg = f"✅ Uploaded: {filename} ({len(new_df)} rows)"
        return new_df.to_dict("records"), charts, msg
    except Exception as e:
        return df.to_dict("records"), None, f"❌ Error loading file: {e}"
