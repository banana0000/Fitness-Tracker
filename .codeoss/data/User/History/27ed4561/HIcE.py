...
form_layout = dbc.Container(
    [
        ...
        dbc.Card(
            dbc.CardBody(
                [
                    html.P("Enter your daily steps. Calories will be calculated automatically.",
                           className="text-light"),
                    dbc.Row(
                        [
                            dbc.Col(
                                dbc.Input(id="input-day", placeholder="Day (e.g. Monday)", type="text"),
                                md=6,
                            ),
                            dbc.Col(
                                dbc.Input(id="input-steps", placeholder="Steps", type="number"),
                                md=6,
                            ),
                        ],
                        className="mb-3",
                    ),
                    dbc.Button("Add Entry", id="add-btn", n_clicks=0,
                               color="warning", className="text-dark mb-3"),
...
def add_entry(n_clicks, day, steps, current):
    ...
    # 🔥 Calories estimated automatically:
    calories = int(int(steps) * 0.05)

    new_row = {"Day": day, "Steps": int(steps), "Calories": calories}
    df_local = df_local.append(new_row, ignore_index=True)
